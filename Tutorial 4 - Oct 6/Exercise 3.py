...
def delta(V_mem, i, method):
    if method == 1:
        ...
        #this is the bag method
        #returns a total output based on this method
...
def simulate(t_end, method):
    sim_time=0
    global V_membrane
    while(sim_time < t_end):
        output_neuron = lambda_dtss(V_membrane)
        V_membrane = delta(V_membrane, sim_time, method)
...
def main():
    method_input = int(input('What method should we use for the inputs? (1, 2, or 3):'))
    t_end = 7.0
    simulate(t_end, method_input)
